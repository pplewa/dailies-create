...
exports.getReporter = function() {
	...
	request({ 
		...
		request({ url: reportUrl, json: true }, function (error2, response2, data2) {
			...
			var sleep = JSONSelect.match('.responses .questionPrompt:val("How did you sleep?") ~ .answeredOptions string', data2)[0];
			...
			// morning
			var goal = JSONSelect.match('.responses .questionPrompt:val("What is your goal for today?") ~ .textResponses', data2)[0];
			var dreams = JSONSelect.match('.responses .questionPrompt:val("Did you remember any dreams?") ~ .textResponses', data2)[0];
