...
exports.getReporter = function() {
	...
	request({ 
		...
		var happy = JSONSelect.match('.responses .questionPrompt:val("How happy are you? 1-10") ~ .numericResponse', data);
		var relaxed = JSONSelect.match('.responses .questionPrompt:val("How relaxed are you? 1-10") ~ .numericResponse', data);
		var farts = JSONSelect.match('.responses .questionPrompt:val("How many farts?") ~ .numericResponse', data)[0];
		var drinks = JSONSelect.match('.responses .questionPrompt:val("How many drinks?") ~ .numericResponse', data)[0];
		var freak = JSONSelect.match('.responses .questionPrompt:val("How many times did I freak out?") ~ .numericResponse', data)[0];
		var sex = JSONSelect.match('.responses .questionPrompt:val("Did I have sex today?") ~ .answeredOptions string', data)[0];
		var eatSlow = JSONSelect.match('.responses .questionPrompt:val("Did I eat slow today?") ~ .answeredOptions string', data)[0];
		var pomodoros = JSONSelect.match('.responses .questionPrompt:val("How many pomodoros did you do today?") ~ .numericResponse', data)[0];
		...
		if (goal && goal[0] && goal[0].text) {
			reporterData.push({ name: 'Goal', 'value': goal[0].text });
		}

		if (dreams && dreams[0] && dreams[0].text) {
			reporterData.push({ name: 'Dreams', 'value': dreams[0].text });
		}
